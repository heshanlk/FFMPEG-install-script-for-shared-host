...
tar -xvjf $_package
cd MPlayer-1.0rc1/
./configure --prefix=$HOME --with-codecsdir=$HOME/lib/codecs/  \
	--with-extraincdir=$HOME/include --with-extralibdir=$HOME/lib

make -j$cpu
make install
cp -f etc/codecs.conf $HOME/etc/mplayer/codecs.conf
